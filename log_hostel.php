...
function login(){
	...
	if(uname==''){
		...
		document.ilim.action='log_hostel.php?log=LOG';
...
<script language="javascript">
	function do_ok(type){
		if(type==1){
			...
			document.location.href="log_hostel.php";
		}
	}
</script>
</head>
<?php	
	...
	$sql = "SELECT * FROM _tbl_user WHERE is_hostel=1 AND f_isdeleted=0 AND f_userid=".tosql($varUser,"Text")." AND f_password=".tosql(md5($varPswd),"Text");
	...
	if(!$rslogin->EOF){
		...
		*/
		...
		audit_log($msg,'hostel',$varUser,'ERR');
		/*print '<script>
