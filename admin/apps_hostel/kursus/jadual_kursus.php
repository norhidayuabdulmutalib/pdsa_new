...
$sSQL = "SELECT A.* FROM _tbl_kursus_jadual A, _tbl_kursus B WHERE A.is_deleted=0 AND A.courseid=B.id AND A.enddate>=".tosql(DBDate($tkh_mula));
...
$sSQL .= " UNION ";
$sSQL .= "SELECT * FROM _tbl_kursus_jadual WHERE is_deleted=0 AND acourse_name<>'' AND enddate>=".tosql(DBDate($tkh_mula));
