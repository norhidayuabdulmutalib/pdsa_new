...
<?php } else {
	...
	if(empty($id_kur_det)){
		...
		$rs = &$conn->Execute($sql);
		$id_kur_det = $conn->Insert_ID(); //mysql_insert_id();
	} else {
	...
	//print $id;
	if(!empty($_FILES['file1']['name'])){ $img_baru='Y'; } else { $img_baru='N'; }
	//print $_FILES['file1'];
	if($img_baru=='Y'){

		//set_time_limit(60);// make reasonably sure the script does not time out on large files
		$upload_dir = "/var/www/html/itis/admin/temp/"; 
		$new_file = $_FILES['file1'];
		//echo "Mana fail-->".$new_file['name']."/".$new_file['size'];
		$file_name = str_replace(" ","_",$new_file['name']); 
		$file_size = $new_file['size']; 
		$file_tmp = $new_file['tmp_name'];
		//echo "Mana fail--> [".$file_size."/".$_FILES['file1']['tmp_name']."] <br><br>";
		$file_extension = end(explode(".", $file_name));
		//echo "<br>File Ext-->".$file_extension;
	
		$allowedExtensions = array("jpg","jpeg","gif","png","pdf","doc","xls","docx","txt","ppt","pptx","xlsx"); 
		foreach ($_FILES as $file) { 
		//print '<br>'.$file['tmp_name'];
		if ($file['tmp_name'] > '') { 
		...
		switch( $file_extension ) { 
			...
			case "xlsx": $jenisfail1="application/vnd.ms-excel"; break; 
			case "txt": $jenisfail1="text/plain"; break; 
			case "ppt": $jenisfail1="application/vnd.ms-powerpoint"; break; 
			case "pptx": $jenisfail1="application/vnd.ms-powerpoint"; break; 
		...
		$target = "/var/www/html/itis/admin/temp/"; 
		//$target = ""; 
		$target = $target . basename($file_name); 
		$ok=1; 
		print $file_tmp." / ".$target;
		if(move_uploaded_file($uploadedfile, $target)) { 
			echo "The file ". basename($uploadedfile). " has been uploaded"; 
		} else { 
		...
		$conn->execute($sSQL);
		if(mysql_errno()!= 0){ print 'Error1 : '.mysql_error(); exit; }
	}
	//exit;
	print "<script language=\"javascript\">
		...
		$sSQLD = "DELETE FROM _tbl_kursus_det WHERE id_kur_det='$id_kur_det'";
		$conn->execute($sSQLD);
		if(mysql_errno()!= 0){ print 'Error : '.mysql_error(); exit; }
	//exit;
	print "<script language=\"javascript\">
		alert('Dokumen yang dipilih telah dihapuskan');
		//parent.location.reload();	
		refresh = parent.location; 
		parent.location = refresh;
		</script>";
