...
$URLs = str_replace("jakim","islam",$URLs);
...
$sql='';
$data_peserta=0;
if(!empty($rs_det->fields['jtb1']) && !empty($rs_det->fields['jtb1'])){ 
	$data_peserta=0;
	//$sql .= " AND ".date("Y")."-year(f_peserta_appoint_dt)".$rs_det->fields['jtb1'] . $rs_det->fields['jtb2']; 
}
if(!empty($rs_det->fields['grade1']) && !empty($rs_det->fields['grade1'])){ 
	$data_peserta=0;
	$sql .= " AND f_title_grade1 ".$rs_det->fields['grade1'] . tosql($rs_det->fields['grade2']);
}
if(!empty($rs_det->fields['subjek'])){ 
	$data_peserta=1;
	$sql .= " AND D.id=".tosql($rs_det->fields['subjek']); 
}
...
if($data_peserta==1){
	$sSQL="SELECT A.* FROM _tbl_peserta A, _tbl_kursus_jadual_peserta B, _tbl_kursus_jadual C, _tbl_kursus D 
		WHERE A.f_peserta_noic=B.peserta_icno AND B.Eventid=C.id AND C.courseid1=D.courseid AND A.is_deleted=0 AND A.f_peserta_nama<>'' ";
	if(!empty($gred)){ $sSQL.=" AND A.f_title_grade=".tosql($gred,"Number"); } 
	if(!empty($jabatan)){ $sSQL.=" AND A.BranchCd=".tosql($jabatan); } 
	if(!empty($search)){ $sSQL.=" AND (A.f_peserta_nama LIKE '%".$search."%' OR A.f_peserta_noic LIKE '%".$search."%') "; }
	$sSQL .= $sql;
	$sSQL .= " AND A.f_peserta_noic NOT IN (SELECT peserta_icno FROM _tbl_kursus_jadual_peserta WHERE EventId=".tosql($id).")";
	$sSQL .= " ORDER BY A.f_peserta_nama";
} else {
	$sSQL="SELECT * FROM _tbl_peserta WHERE is_deleted=0 AND f_peserta_nama<>'' ";
...
<table width="96%" border="1" cellpadding="3" cellspacing="0" align="center">
	...
	<?php while(!$rs->EOF){ $bil++; 
		...
		$rs_data = &$conn->query($sql_k); //print $sql_k;
		if(!$rs_data->EOF){ 
			$ada = 'A'; $sel=9; 
	...
	<tr>
        ...
        <td align="left"><a href="#" onclick="openModal('peserta/biodata_update_view.php?id=<?=$ids;?>&ic=<?=$ic;?>&ty=PE')"><?php print $rs->fields['f_peserta_nama'];?></a><?=$tada;?></td>
        ...
        <td align="center" bgcolor="<?=$bgcolor;?>"><?=$datang;?></td>
        <td align="center" bgcolor="<?=$bgcolor;?>"><?=$xdatang;?></td>
        <td align="center" bgcolor="<?=$bgcolor;?>"><?=$tjawab;?></td>
    </tr>
    <?php $rs->movenext(); } ?>
    <tr>
    	<td colspan="8">
...
<?php } else {
	...
	for($i=0;$i<$size;$i++){
		...
		$rs_get = &$conn->query($sql_sel);
		if($rs_get->EOF){
			$idpeserta = dlookup("_tbl_peserta","id_peserta","f_peserta_noic='{$icno}'");
			$sqlu = "INSERT INTO _tbl_kursus_jadual_peserta(id_peserta, EventId, peserta_icno, InternalStudentSelectedDt, InternalStudentAccepted, 
			InternalStudentInputDt, InternalStudentInputBy, is_selected) 
			VALUES(".tosql($idpeserta).", ".tosql($event_id).", ".tosql($icno,"Text").", ".tosql(date("Y-m-d H:i:s")).", 0, 
	...
	audit_trail("Tambah maklumat peserta","INSERT");
