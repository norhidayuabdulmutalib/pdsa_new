...
if(!empty($ids) && !empty($pro)){
	...
	if($pro=='ALL'){
		...
		WHERE A.peserta_icno=B.f_peserta_noic AND A.is_deleted=0 AND A.is_selected in (0) AND A.EventId=".tosql($id);
		$sSQL .= " GROUP BY B.f_peserta_noic ORDER BY A.is_selected, B.f_peserta_nama";
		$rs = &$conn->query($sSQL);
		//print $sSQL;
		while(!$rs->EOF){
			$idss = $rs->fields['InternalStudentId'];
			$sqlu ="UPDATE _tbl_kursus_jadual_peserta SET is_selected=9 WHERE InternalStudentId=".tosql($idss);
			//print "<br>".$sqlu;
			$conn->Execute($sqlu);
			$rs->movenext();
		}
...
<table width="96%" border="0" cellpadding="1" cellspacing="0" align="center">
	<tr>
        <td align="left">
            ...
            <input type="button" value="Tutup" onclick="form_back()" style="cursor:pointer" /><br />
            <br /><img src="../images/off.gif" />&nbsp;&nbsp;Sila klik untuk menghantar mesej tidak berjaya
        	</div>
...
<div style="position: absolute; left: 1px; top: 90px; width: 100%; height: 90%; background-color: #ffffff; overflow: auto;">
<table width="96%" border="1" cellpadding="3" cellspacing="0" align="center">
	...
	<tr bgcolor="#CCCCCC">
    	...
    	<td align="center" width="5%" rowspan="2">Status<br />
        	<br /><img src="../images/off.gif" style="cursor:pointer" title="Sila klik untuk proses tidak berjaya" 
            onclick="chk_pilih('modal_form.php?<?=$URLs;?>','','ALL')" />
        </td>
    </tr>
    <tr bgcolor="#CCCCCC">
	...
	<?php while(!$rs->EOF){ $bil++; 
		...
		else if($datang>$tjawab || $datang>$xdatang){ $bgcolor = "#9999CC"; }
		else if($xdatang>$datang || $xdatang>$tjawab){ $bgcolor = "#CCCC99"; }
		else { $bgcolor = "#FFFFFF"; }
	?>
	<tr>
        ...
        <td align="left"><?php print dlookup("_ref_tempatbertugas","f_tempat_nama","f_tbcode=".tosql($rs->fields['BranchCd']));?>
        <?php if($_SESSION["s_logid"]=='admin'){ print "<br>".$rs->fields['InternalStudentId']; }?></td>
        ...
        <td align="center" bgcolor="<?=$bgcolor;?>"><?=$dp;?></td>
