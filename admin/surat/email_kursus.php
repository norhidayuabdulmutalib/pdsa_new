...
/* additional headers */
$headers .= "From: Webmaster ITIS<kursus_ilim@islam.gov.my>\r\n";
...
//print $mail_to.':'.$kandungan_email; exit;

// EMAIL KEPADA PESERTA
//$conn->debug=true;
$rss = $conn->execute("SELECT * FROM _tbl_surat WHERE surat_jenis='T' AND peserta_icno=".tosql($f_peserta_noic,"Text"). " 
	AND EventId=".tosql($EventId));
if($rss->EOF){
	$sqle = "INSERT INTO _tbl_surat(peserta_icno, EventId, surat_jenis, surat_tarikh, surat, create_by, create_dt) ";
	$sqle .= "VALUES(".tosql($f_peserta_noic).", ".tosql($EventId).", ".tosql("T").", ".tosql(date("Y-m-d")).", 
	".tosql($surat_tawaran).", ".tosql($by).", ".tosql(date("Y-m-d H:i:s")).")";
	$conn->Execute($sqle);
} else {
	$sqle = "UPDATE _tbl_surat SET surat=".tosql($surat_majikan).", create_by=".tosql($by).", create_dt=".tosql(date("Y-m-d H:i:s"))." 
...
$headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
/* additional headers */
$headers .= "From: Eebmaster ITIS<kursus_ilim@islam.gov.my>\r\n";
//$headers .= "Cc:nizamms@gmail.com\r\n";
