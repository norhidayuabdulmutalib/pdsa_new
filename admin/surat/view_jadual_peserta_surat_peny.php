...
$href_surat = "modal_print.php?win=".base64_encode('surat/view_jadual_peserta_surat_peny.php;'.$rs->fields['id']);
$href_surat = "surat/view_jadual_peserta_surat_peny.php?prn=WORD&id=".$rs->fields['id'];
?>
<?php if($prn<>'WORD'){ ?>
...
$surat='';
while(!$rs->EOF){ $bil++; 
	$surat = stripslashes($rs->fields['surat_majikan']);
	print $surat;
	$rs->movenext();
} 
if(empty($surat)){
