...
if(empty($id_peserta)){
	...
	if($rsp->EOF){
		...
		//exit;
		$url = base64_encode($userid.';apps/peserta/peserta_form.php;peserta;peserta;'.$id_peserta);
	...
	if($proses=='HAPUS'){
		...
		//exit;
		if(!$result){ echo "Invalid query : " . mysql_error(); exit; }
		$url = base64_encode($userid.';apps/peserta/peserta_list.php;peserta;peserta');
		echo "<meta http-equiv=\"refresh\" content=\"1; URL=index.php?data=".$url."\">";
	} else {
		echo "Update";
