...
if($prn=='WORD'){
	...
	header("content-disposition: attachment;filename=borang_kehadiran.doc");
}
$conn->debug=true;
$proses = $_POST['proses'];
$ty = $_GET['ty'];
...
<?
$href_surat = "apps/modal_print.php?win=".base64_encode('kursus/jadual_peserta_kehadiran.php;'.$rs->fields['id']);
?>
...
$surat='';
while(!$rs->EOF){ $bil++; 
	$surat = stripslashes($rs->fields['surat_jawapan']);
	print $surat;
	$rs->movenext();
} 
if(empty($surat)){
