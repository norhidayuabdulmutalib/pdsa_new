...
if($types=='NEXT'){
	...
	$sSQL="SELECT A.*, B.courseid, B.coursename, B.SubCategoryCd FROM _tbl_kursus_jadual A, _tbl_kursus B WHERE A.courseid=B.id AND B.is_deleted=0 
	AND month(startdate)='$this_mth' AND year(startdate)='$this_year' AND A.category_code=1 AND startdate>=".tosql(date("Y-m-d"));
	...
	FROM _tbl_kursus_jadual A WHERE month(startdate)='$this_mth' AND year(startdate)='$this_year' AND category_code=2 AND startdate>=".tosql(date("Y-m-d"));
