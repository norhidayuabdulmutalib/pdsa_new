...
<table width="98%" align="center" cellpadding="0" cellspacing="0" border="0">
	...
	$sql1 = "SELECT * FROM _tbl_kursus_jadual_peserta WHERE EventId=".tosql($id)." GROUP BY peserta_icno";
	...
	WHERE A.id_peserta=B.InternalStudentId AND B.InternalStudentAccepted=1 AND A.is_nilai=1 AND A.event_id=".tosql($id);
	...
	<tr>
        ...
        <?php //$conn->debug=true;
			$sqlsel = "SELECT A.* FROM _tbl_set_penilaian_peserta A, _tbl_kursus_jadual_peserta B 
			WHERE A.id_peserta=B.InternalStudentId AND B.InternalStudentAccepted=1 AND A.is_nilai=0 AND A.event_id=".tosql($id);
	...
	<tr><td colspan="5"><table width="100%" border="0" cellspacing="0" cellpadding="1">
      ...
      <tr>
		...
		<table width="100%" border="1" cellpadding="5" cellspacing="0">
			...
			<?php
            $cnt=0; $cnt_jum=0;//$conn->debug=true; //AND A.is_sijil=1
            $sqljlain = "SELECT count(*) as grades, B.f_title_grade FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B
				...
				AND A.EventId=".tosql($id)." GROUP BY B.f_title_grade";
        ...
        <td colspan="1" valign="top">
            <table width="100%" border="1" cellpadding="5" cellspacing="0">
                ...
                $sqljlain = "SELECT count(*) as negeris, B.f_peserta_negeri FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B
                            WHERE A.peserta_icno=B.f_peserta_noic AND A.InternalStudentAccepted=1 AND A.is_selected IN (1,9) AND A.EventId=".tosql($id)." 
							AND B.is_deleted=0 GROUP BY B.f_peserta_negeri";
                //print $sqljlain;
				$rsjlain = &$conn->Execute($sqljlain); 
...
$fset_id = $rs->fields['fset_id'];
    ...
    <tr>
		...
		while(!$rsrem->EOF){
			print '<li>'. stripslashes(nl2br($rsrem->fields['fsetbp_remarks'])).'</li>';
		$rsrem->movenext(); }
		print '</ul>';
		}
		?>
    ...
    <tr>
		...
		while(!$rsrem->EOF){
			print '<li>'.stripslashes(nl2br($rsrem->fields['fsetp_remarks'])).'</li>';
		$rsrem->movenext(); }
		print '</ul>';
		}
		?>
