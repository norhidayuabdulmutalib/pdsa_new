...
if(empty($id)){
	...
	create_dt, create_by)
	VALUES(".tosql($id,"Text").", ".tosql($insname,"Text").", ".tosql($p_jantina,"Text").", ".tosql($titlegredcd,"Text").", '{$position}',  
	...
	audit_trail($sql,"Penceramah Insert");
	/* if($is_user=='1'){
		$pass = md5("123");
	...
	$url = base64_encode($userid.';penceramah/penceramah_form.php;penceramah;daftar;'.$id);
	...
	if($proses=='HAPUS'){
		...
		//exit;
		if(!$result){ echo "Invalid query : " . mysql_error(); exit; }
		$url = base64_encode($userid.';apps/penceramah/penceramah_list.php;penceramah;daftar');
		echo "<meta http-equiv=\"refresh\" content=\"1; URL=index.php?data=".$url."\">";
		...
		if(!$result){ echo "Invalid query : " . mysql_error(); exit; }
		audit_trail($sql,'Penceramah Update');
		/* if($is_user=='1'){
		...
		//exit;
		$url = base64_encode($userid.';penceramah/penceramah_form.php;penceramah;daftar;'.$id);
		//$url = base64_encode($userid.';penceramah/penceramah_form.php;penceramah;daftar;');
		echo "<meta http-equiv=\"refresh\" content=\"1; URL=index.php?data=".$url."\">";
	}
}
?>