<?php 
	...
	if(empty($id)){
		...
		$rs = &$conn->Execute($sql);
		$id = mysql_insert_id();
		$id = dlookup("_tbl_instructor_akademik","ingenid_akademik","ingenid=".tosql($_SESSION['ingenid'])." 
			AND inaka_sijil=".tosql($inaka_sijil)." AND inaka_kursus=".tosql($inaka_kursus));
		audit_trail($sql,"akademik");
	} else {
