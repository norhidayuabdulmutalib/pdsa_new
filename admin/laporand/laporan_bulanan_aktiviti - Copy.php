...
<?
	...
	if(!empty($bulan)){ $sqld .= " AND month(A.startdate)=".tosql($bulan); }
	if(!empty($kursus)){ $sqld .=" AND A.sub_category_code=".tosql($kursus); }
	//$conn->debug=true;
	$rskursus = &$conn->execute($sqld);	
	//$conn->debug=false;

	if(!$rskursus->EOF){ 
		$bil=0;
		while(!$rskursus->EOF){ $bil++;  $penceramah='';
			//$kos = number_format($rsdata->fields['jumkos_sebenar'],2);
			//$kos_ceramah = number_format($rsdata->fields['jumkceramah_sebenar'],2);
			$kos = number_format($rskursus->fields['jumkos'],2);
			$kos_ceramah = number_format($rskursus->fields['jumkceramah'],2);
			if($rskursus->fields['category_code']=='1'){ $kursus = dlookup("_tbl_kursus","coursename","id=".tosql($rskursus->fields['courseid'])); }
			else { $kursus = $rskursus->fields['acourse_name']; }
			$tarikh = DisplayDate($rskursus->fields['startdate'])."<br>-<br>".DisplayDate($rskursus->fields['enddate']);
			$sqlp = "SELECT B.insname, B.insorganization FROM _tbl_kursus_jadual_det A, _tbl_instructor B WHERE A.instruct_id=B.ingenid AND A.event_id=".tosql($rskursus->fields['id']);
...
//$conn->debug=true;
			...
			A.InternalStudentAccepted=1 AND B.is_deleted=0 AND A.is_deleted=0 AND A.EventId=".tosql($rskursus->fields['id']);
			...
			<tr>
                ...
                <?php if($lain>0){ 
					...
					/*$sqljlain = "SELECT distinct B.BranchCd FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
						WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND D.ref_ktid>=5 AND B.is_deleted=0  AND A.is_selected=1 AND  
						A.InternalStudentAccepted=1 AND A.EventId=".tosql($rskursus->fields['id']);
					*/
					$sqljlain = "SELECT distinct B.BranchCd FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
						...
						AND B.is_deleted=0 AND A.is_deleted=0 AND A.EventId=".tosql($rskursus->fields['id']);
					$sqljlain .= " GROUP BY A.peserta_icno";	
...
//$conn->debug=true;
						...
						WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND D.ref_ktid IN (5,6) AND B.is_deleted=0 AND A.is_selected=1 
						AND A.InternalStudentAccepted=1 AND A.EventId=".tosql($rskursus->fields['id'])." AND B.BranchCd=".$rsjlain->fields['BranchCd'];
						$sqljlain .= " GROUP BY A.peserta_icno";
...
//$conn->debug=false;
				 ...
				 $cnts++;
				 $sqlj = "SELECT A.* FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B
					WHERE A.peserta_icno=B.f_peserta_noic AND A.InternalStudentAccepted=1 AND B.BranchCd='0099' AND A.is_selected=1   
					AND A.EventId=".tosql($rskursus->fields['id']);
