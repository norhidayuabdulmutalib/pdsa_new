...
for($i=1;$i<=12;$i++){
	...
	if(!$rs->EOF){
		...
		while(!$rs->EOF){
			...
			if($category_code==1){
				$sql = "SELECT count(*) AS jumi FROM _sis_a_tblasrama WHERE jantina='L' AND A.InternalStudentAccepted=1 AND A.is_sijil=1 AND event_id=".tosql($id);
				$rskursus = &$conn->execute($sql);	
				$ilim_lelaki += $rskursus->fields['jumi'];
				$sql = "SELECT count(*) AS jumi FROM _sis_a_tblasrama WHERE jantina='P' AND A.InternalStudentAccepted=1 AND A.is_sijil=1 AND event_id=".tosql($id);
				$rskursus = &$conn->execute($sql);	
				$ilim_perempuan += $rskursus->fields['jumi'];
			} else if($category_code==2){
				$sql = "SELECT count(*) AS jumi FROM _sis_a_tblasrama WHERE jantina='L' AND A.InternalStudentAccepted=1 AND A.is_sijil=1 ND event_id=".tosql($id);
				$rskursus = &$conn->execute($sql);	
