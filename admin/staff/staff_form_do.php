...
if($f_level==99){ $is_admin=1; $is_domestik=1; }
else if($f_level==1){ $is_admin=1; $is_domestik=1; }
...
if(empty($id)){
	...
	$id = "ILIM_". uniqid();
	$sql = "INSERT INTO _tbl_user(id_user, f_staffid, f_noic, f_name, 
	...
	VALUES(".tosql($id,"Text").", ".tosql($id,"Text").", ".tosql($f_noic,"Text").", ".tosql($f_name,"Text").", 
	...
	//$url = base64_encode($userid.';apps/staff/staff_list.php;admin;staff;');
	$url = base64_encode($userid.';staff/staff_form.php;admin;staff;'.$id);
	...
	if($proses=='HAPUS'){
		...
		if(!$result){ echo "Invalid query : " . mysql_error(); exit; }
		audit_trail($sql,"");
		$url = base64_encode($userid.';staff/staff_list.php;admin;staff;;');
		echo "<meta http-equiv=\"refresh\" content=\"1; URL=index.php?data=".$url."\">";
	} else {
		//$gred = dlookup("ref_gred","gred","gred_id=".tosql($gred_id,"Number"));
		...
		", f_updateby=".tosql($_SESSION["s_userid"],"Text").", is_admin='{$is_admin}', is_hostel='{$is_domestik}' ". 
		" WHERE id_user=".tosql($id,"Text");
