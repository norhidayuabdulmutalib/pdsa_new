...
if(empty($id)){
	...
	".tosql(date("Y-m-d H:i:s"),"Text").", ".tosql($_SESSION["s_userid"],"Text").",".tosql($user,"Text").",". tosql(md5("123")).", 
	".$is_admin.",".$is_domestik.",".$f_jabatan.")";
	$result = $conn->Execute($sql);
	if(!$result){ echo "Invalid query : " . mysql_error(); exit; }
	//$url = base64_encode($userid.';apps/staff/staff_list.php;admin;staff;');
	$url = base64_encode($userid.';apps/staff/staff_form.php;admin;staff;'.$id);
	...
	if($proses=='HAPUS'){
		...
		//$gred = dlookup("ref_gred","gred","gred_id=".tosql($gred_id,"Number"));
		$sql = "UPDATE _tbl_user SET f_name=".tosql($f_name,"Text"). ", f_noic=".tosql($f_noic,"Text"). ", f_jabatan=".tosql($f_jabatan,"Text").
