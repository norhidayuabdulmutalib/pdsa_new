...
$sql = "SELECT * FROM _ref_titlegred A, _tbl_peserta B 
WHERE A.f_gred_id=B.f_title_grade AND A.f_status=0 AND A.is_deleted=0 
GROUP BY A.f_gred_id ORDER BY A.f_gred_code";
$rstitle = &$conn->execute($sql);
$sqltt= " AND B.BranchCd='0016' ";
if(!empty($kampus_id)){
	$sqltt .= " AND kampus_id=".tosql($kampus_id); 
} else {
	$sqltt .= " AND kampus_id=".tosql($_SESSION['KAMPUS_PRINT']); 
}
//$conn->debug=false;
if(!$rstitle->EOF){
	while(!$rstitle->EOF){
	...
	WHERE A.peserta_icno=B.f_peserta_noic AND A.EventId=C.id AND A.is_sijil=1 AND datediff( enddate, startdate ) > 6 
	...
	WHERE B.f_title_grade=".tosql($rstitle->fields['f_gred_id'])." AND B.BranchCd='0016'";
	$sql3 .= " AND B.f_peserta_noic NOT IN (SELECT peserta_icno FROM _tbl_kursus_jadual_peserta A, _tbl_kursus_jadual C
	WHERE A.Eventid=C.id AND A.is_sijil=1)"; //.$sqltt;
