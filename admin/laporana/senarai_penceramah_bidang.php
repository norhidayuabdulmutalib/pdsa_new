...
$strPusat=((strlen($penceramah)>0)?" AND iA.ngenid = ".tosql($penceramah)." ":"");
$strGred=((strlen($inskategori)>0)?" AND A.inskategori = ".tosql($inskategori)." ":"");
...
<table width="<?=$width?>" border="0" align="center" cellpadding="3" cellspacing="0">
    ...
    <tr><td  width="30%"align="right">Kategori Penceramah : </td>
        ...
        <select name="inskategori"  onchange="do_post('PRO')" style="cursor:pointer" title="Sila buat pilihan untuk penyenaraian nama penceramah">
		...
		<tr>
            ...
            <td>
                <select name="inpakar_bidang" onChange="do_post('PRO')" >
                <option value=""></option>
                <?php 
       ...
       // if(!empty($inpakar_bidang)){ $sqlp .= " AND inskategori=".tosql($inpakar_bidang); } 
        //$sqlp .= " ORDER BY insname";

		$sqlp = "SELECT * FROM _tbl_instructor A ";
		if(!empty($inpakar_bidang)){ $sqlp .= ", _tbl_instructor_kepakaran B "; }
		$sqlp .= " WHERE A.is_deleted=0 ";
		if(!empty($inskategori)){ $sqlp .= " AND A.inskategori = ".tosql($inskategori); }
		if(!empty($inpakar_bidang)){ $sqlp .= " AND A.ingenid=B.ingenid AND B.inpakar_bidang=".tosql($inpakar_bidang); }
		//$sql .= $strPusat . $strGred;
		$sqlp .= " ORDER BY A.insname";
...
if(!empty($inskategori)){ $sql .= " AND A.inskategori = ".tosql($inskategori); }
if(!empty($penceramah)){ $sql .= " AND A.ingenid=".tosql($penceramah); }
...
?>
    <tr height="25">
		...
		<?php while(!$rs1->EOF){  $bilg++; ?>
                <?php if($bilg>1){ print '<br><hr>'; } ?>
