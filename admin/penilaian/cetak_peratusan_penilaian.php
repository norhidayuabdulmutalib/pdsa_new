...
//$conn->debug=true;
$sSQL="SELECT A.courseid, A.coursename, B.categorytype, C.SubCategoryNm, C.id AS CID, D.startdate, D.enddate, D.penyelaras 
...
<table width="98%" align="center" cellpadding="0" cellspacing="0" border="0">
  ...
  <TBODY style="width:100%">
    ...
    <tr><td colspan="5">
        <table width="100%" cellpadding="2" cellspacing="0" border="0" align="center">
            ...
            <tr>
                ...
                <td align="center"><b> : </b></td>
                <td align="left"><?php print dlookup("_tbl_kursus_catsub","SubCategoryDesc","id=".tosql($rskursus->fields['CID'])); //print $rskursus->fields['SubCategoryNm'];?></td>                
            </tr>
		</table>
    </td></tr>
	...
	//$jum_nilai = dlookup_cnt("_tbl_set_penilaian_peserta","distinct id_peserta","is_nilai=1 AND event_id=".tosql($id)." GROUP BY id_peserta");	
	$sqlsel = "SELECT A.* FROM _tbl_set_penilaian_peserta A, _tbl_kursus_jadual_peserta B 
	WHERE A.id_peserta=B.InternalStudentId AND B.InternalStudentAccepted=1 AND B.is_selected IN (1,9) AND A.is_nilai=1 AND A.event_id=".tosql($id);
	...
	if(!empty($jum_peserta)){ $pct_jum_pumur = ($jum_pumur/$jum_peserta)*100; }
	...
	if(!empty($jum_peserta)){ $pct_jum_jantina = ($jum_jantina/$jum_peserta)*100; }
	...
	if(!empty($jum_peserta)){ $pct_jum_jawatan = ($jum_jawatan/$jum_peserta)*100; }
	...
	if(!empty($jum_peserta)){ $pct_jum_kkursus = ($jum_kkursus/$jum_peserta)*100; }
	?>
	...
	<tr><td colspan="5"><table width="100%" border="1" cellpadding="5" cellspacing="0">
    	...
    	<tr>
            ...
            <td width="85%" align="left">Nama Jabatan/Kementerian</td>	    
            <td width="10%" align="center">Jumlah Peserta</td>
        </tr>
        <tr>
        	<td align="right">1.</td>	
            <td>Jabatan Kemajuan Islam Malaysia (JAKIM)</td>
            <td align="center"><?php $jum_all=0; 
            $sqlj = "SELECT distinct A.* FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
			WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND A.is_selected IN (1,9) AND D.ref_ktid=1 
			AND A.InternalStudentAccepted=1 AND B.is_deleted=0 AND A.EventId=".tosql($id);
			$sqlj .= " GROUP BY A.peserta_icno";
			//print $sqlj; //AND A.is_sijil=1 
			$rsj = $conn->query($sqlj); $jakim = $rsj->recordcount(); print $jakim; $jum_all+=$jakim;
			?></td>
        </tr>    
        <tr>
        	<td align="right">2.</td>	
            ...
            <td align="center"><?php
            $sqlj = "SELECT distinct A.* FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
			WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND A.is_selected IN (1,9) AND D.ref_ktid=2 
			AND  A.InternalStudentAccepted=1 AND B.is_deleted=0 AND A.EventId=".tosql($id);
			$sqlj .= " GROUP BY A.peserta_icno";
			//print $sqlj; // A.is_sijil=1 AND
			$rsj = $conn->query($sqlj); $jain = $rsj->recordcount(); print $jain; $jum_all+=$jain;
			?></td>
        </tr>    
        <tr>
        	<td align="right">3.</td>	
            ...
            <td align="center"><?php
			$sqlj = "SELECT distinct A.* FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
			WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND A.is_selected IN (1,9) AND D.ref_ktid=3 
			AND A.InternalStudentAccepted=1 AND B.is_deleted=0 AND A.EventId=".tosql($id);
			$sqlj .= " GROUP BY A.peserta_icno";
			$rsj = $conn->query($sqlj); $main = $rsj->recordcount();	print $main; $jum_all+=$main;	//AND A.is_sijil=1 
			?></td>
        </tr>    
        <tr>
        	<td align="right">4.</td>	
            ...
            <td align="center"><?php
            $sqlj = "SELECT distinct A.* FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
			WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND A.is_selected IN (1,9) AND D.ref_ktid=4 
			AND A.InternalStudentAccepted=1 AND B.is_deleted=0 AND A.EventId=".tosql($id);
			$sqlj .= " GROUP BY A.peserta_icno";
			$rsj = $conn->query($sqlj); $jmufti = $rsj->recordcount(); print $jmufti; $jum_all+=$jmufti; //AND A.is_sijil=1 
			?></td>
        </tr> 
        <?php
		$cnts=5;
        $sqljlain = "SELECT distinct B.BranchCd FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
			WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND A.is_selected IN (1,9) AND D.ref_ktid>=5 
			AND A.InternalStudentAccepted=1 AND B.is_deleted=0 AND A.EventId=".tosql($id);
		$rsjlain .= " GROUP BY A.peserta_icno";
        ...
        <tr>
			...
			?></td>
            <td align="center"><?php
				$sqljlain = "SELECT distinct A.* FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B, _ref_tempatbertugas C, _ref_tempat_kategori D
				WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND A.is_selected IN (1,9) 
				AND A.InternalStudentAccepted=1 AND D.ref_ktid>=5  
				AND B.is_deleted=0 AND A.EventId=".tosql($id)." AND B.BranchCd=".$rsjlain->fields['BranchCd']; //AND A.is_sijil=1
        ...
        <tr>
            ...
            <td>Lain-Lain</td>
            <td align="center"><?php
            $sqlj = "SELECT distinct A.* FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B
			WHERE A.peserta_icno=B.f_peserta_noic AND A.InternalStudentAccepted=1 AND B.BranchCd='0099' AND A.is_selected IN (1,9) 
			AND B.is_deleted=0 AND A.EventId=".tosql($id);
			$sqlj .= " GROUP BY peserta_icno";
			//print $sqlj;
      ...
      <tr>
		...
		<table width="100%" border="1" cellpadding="5" cellspacing="0">
			...
			//WHERE A.peserta_icno=B.f_peserta_noic AND B.BranchCd=C.f_tbcode AND C.ref_ktid=D.ref_ktid AND A.is_selected IN (1,9) AND D.ref_ktid=1 AND A.InternalStudentAccepted=1 
			//AND B.is_deleted=0 AND A.EventId=".tosql($id);
            
			$sqljlain = "SELECT count(*) as grades, B.f_title_grade, A.peserta_icno FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B
                        WHERE A.peserta_icno=B.f_peserta_noic AND A.InternalStudentAccepted=1 AND A.is_selected IN (1,9) 
						AND B.is_deleted=0 AND A.EventId=".tosql($id)." GROUP BY B.f_title_grade";
			//print $sqljlain;
            $rsjlain = $conn->query($sqljlain); 
            while(!$rsjlain->EOF){ $cnt++; 
				...
				$kodgred = $rsgred->fields['f_gred_name'] . " &nbsp;(".$rsgred->fields['f_gred_code'].")".$rsgred->fields['peserta_icno'];
        ...
        <td>&nbsp;</td>
        <td colspan="3" valign="top"><b>Negeri Tempat Bertugas : </b><br />
            <table width="100%" border="1" cellpadding="5" cellspacing="0">
                <tr>
                    <td><b>Bil</b></td>
                    <td><b>Negeri</b></td>
                ...
                $sqljlain = "SELECT count(*) as negeris, B.f_peserta_negeri FROM _tbl_kursus_jadual_peserta A, _tbl_peserta B
                            WHERE A.peserta_icno=B.f_peserta_noic AND A.InternalStudentAccepted=1 AND A.is_selected IN (1,9) 
							AND A.EventId=".tosql($id)." AND B.is_deleted=0 GROUP BY B.f_peserta_negeri";
                $rsjlain = $conn->query($sqljlain); 
				$conn->debug=false;
            ...
            if(!$rs_bhg->EOF) {
                ...
                while(!$rs_bhg->EOF) {
					$bil_bhg++;
					 ...
					 $jump=0;
					 $tot_s1=0; $tot_s2=0; $tot_s3=0; $tot_s4=0; $tot_s5=0; 
            ?>
					...
					$tajuk='';
                    if(!empty($fsetb_pensyarah_id)){ // JIKA MELIBATKAN PENSYARAH
                        $sql_p = "SELECT A.tajuk, A.tarikh, A.masa_mula, A.masa_tamat, B.insname, B.ingenid FROM _tbl_kursus_jadual_masa A, _tbl_instructor B 
						WHERE A.event_id=".tosql($id)." 
						AND A.id_pensyarah=".tosql($fsetb_pensyarah_id)." AND A.id_pensyarah=B.ingenid AND A.id_jadmasa=".tosql($fsetb_jadmasaid);
						...
						$tajuk = stripslashes($rs_pensyarah->fields['tajuk']). " oleh ".stripslashes($rs_pensyarah->fields['insname']);
					} else {
						$tajuk = stripslashes($rs_bhg->fields['nilai_keterangan']);
					}
					...
					$bil=0;
					$jum_set=0;
					$totpct_set1=0;$totpct_set2=0;$totpct_set3=0;$totpct_set4=0;$totpct_set5=0;$totjum_set=0;
					while(!$rs_det->EOF){ 
						$bil++;
						$pct_set1=0;$pct_set2=0;$pct_set3=0;$pct_set4=0;$pct_set5=0;$jum_set=0;
						$fsetdet_id=$rs_det->fields['fsetdet_id'];
						$set1 = $rs_det->fields['fsetdet_1']+$rs_det->fields['fsetdet_jum1'];
						$set2 = $rs_det->fields['fsetdet_2']+$rs_det->fields['fsetdet_jum2'];
						$set3 = $rs_det->fields['fsetdet_3']+$rs_det->fields['fsetdet_jum3'];
						$set4 = $rs_det->fields['fsetdet_4']+$rs_det->fields['fsetdet_jum4'];
						$set5 = $rs_det->fields['fsetdet_5']+$rs_det->fields['fsetdet_jum5'];
						$jum_set=$set1+$set2+$set3+$set4+$set5;
						if(!empty($set1)){ $pct_set1=($set1/$jum_set)*100; $tot_s1+=$set1; $totpct_set1+=$pct_set1; }
						if(!empty($set2)){ $pct_set2=($set2/$jum_set)*100; $tot_s2+=$set2; $totpct_set2+=$pct_set2; }
						if(!empty($set3)){ $pct_set3=($set3/$jum_set)*100; $tot_s3+=$set3; $totpct_set3+=$pct_set3; }
						if(!empty($set4)){ $pct_set4=($set4/$jum_set)*100; $tot_s4+=$set4; $totpct_set4+=$pct_set4; }
						if(!empty($set5)){ $pct_set5=($set5/$jum_set)*100; $tot_s5+=$set5; $totpct_set5+=$pct_set5; }
				...
				?>
                            ...
                            <tr bgcolor="#FFFFFF">
                                ...
                                <td align="center"><strong><?php if(!empty($jum_peserta)){ print number_format(($jum_set/$jum_peserta)*100,2); }?>%</strong></td>
                            </tr>
					...
					$jum_tot=$tot_s1+$tot_s2+$tot_s3+$tot_s4+$tot_s5;
					$jum_total=($tot_s1*1)+($tot_s2*2)+($tot_s3*3)+($tot_s4*4)+($tot_s5*5);
					//$jum_tot = ($tot_s5/200)*100;
					//$jum_display = $jum_tot/15;
					//$jum_tot = 25*20;
					$jumaa = $jum_all*$bil;
					...
					$jum4 = ($tot_s4/$jumaa)*4;
					$jum5 = ($tot_s5/$jumaa)*5;
					$jum_tot=$jum1+$jum2+$jum3+$jum4+$jum5;*/
					//$jum_display = (124/$jumaa)*5;
					//$jum_display = $jum_tot;
					$jum1 = $totpct_set1/5;
					$jum2 = $totpct_set2/4;
					$jum3 = $totpct_set3/3;
					...
					$jum_display = $jum_tot/$bil;
			?>
            	<tr bgcolor="#FFCCFF">
                	<td colspan="3" align="right"><b>Peratusan Keseluruhan Penilaian : <?=$tajuk;?></b></td>
                    <td colspan="2" align="right"><b><?=number_format($jum_display,2);?> %<?//=$tot_s5."/".$jum_all."/".$jum_tot;?></b></td>
                </tr>
            <?php
					$bil = $bil + 1;
				$rs_bhg->movenext();
