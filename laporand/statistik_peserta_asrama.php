...
for($i=$gb;$i<=$mth;$i++){
	...
	if(!$rs->EOF){
		...
		while(!$rs->EOF){
			...
			if($category_code==1){
				$sql = "SELECT count(*) AS jumi FROM _sis_a_tblasrama WHERE jantina='L' AND event_id=".tosql($id);
				$rskursus = &$conn->execute($sql);	
				$ilim_lelaki += $rskursus->fields['jumi'];
				$sql = "SELECT count(*) AS jumi FROM _sis_a_tblasrama WHERE jantina='P' AND event_id=".tosql($id);
				$rskursus = &$conn->execute($sql);	
