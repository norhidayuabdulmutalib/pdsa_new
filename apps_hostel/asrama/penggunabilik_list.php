...
WHERE A.blok_id=B.f_bb_id AND A.is_deleted = 0 AND A.keadaan_bilik=1"; //AND A.status_bilik=0 
...
$href_search = "index.php?data=".base64_encode('user;asrama/menu_asrama.php;asrama;pbilik;;asrama/penggunabilik_list.php');
//$href_search = "index.php?data=".base64_encode('user;asrama/penggunabilik_form.php;asrama;bilik');
?>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr valign="top" bgcolor="#80ABF2"> 
        <td height="30" colspan="3" valign="middle">
        	<font size="2" face="Arial, Helvetica, sans-serif">&nbsp;&nbsp;<strong>SENARAI MAKLUMAT PENGGUNAAN BILIK ASRAMA</strong></font>        </td>
    </tr>
    <tr> 
      <td width="75%" colspan="5"> 
        ...
        if(!$rs->EOF) {
            ...
            while(!$rs->EOF  && $cnt <= $pg) {
				$penghuni=0;
				$href_link = "modal_form.php?win=".base64_encode('asrama/penggunabilik_form.php;'.$rs->fields['bilik_id']);
            ?>
          <tr bgcolor="<? if ($cnt%2 == 1) echo $bg1; else echo $bg2 ?>">
            <td align="right" valign="top"><? echo $bil;?>.&nbsp;</td>
            <td align="left"><a onclick="open_modal('<?=$href_link;?>','Paparan Maklumat Bilik Hostel',1,1)" style="cursor:pointer">
				<? echo stripslashes($rs->fields['no_bilik']);?></a>&nbsp;</td>
