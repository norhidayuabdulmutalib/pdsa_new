...
<table width="100%" align="center" cellpadding="5" cellspacing="1" border="0">
   ...
   <tr>
     ...
     <td colspan="2" align="left">
         ...
         <?	while(!$rs_l->EOF){
		 		//$cnt = dlookup("_sis_a_tblasrama_tempah","count(tempahan_id)","bilik_id=".tosql($rs_l->fields['bilik_id']));
				$sqlte = "SELECT * FROM _sis_a_tblasrama_tempah WHERE bilik_id=".tosql($rs_l->fields['bilik_id'])." 
				AND startdt BETWEEN ".tosql($rs->fields['startdate']). " AND ".tosql($rs->fields['enddate']);
				//$sqlte = "SELECT * FROM _sis_a_tblasrama_tempah WHERE (".tosql($rs->fields['startdate'])." BETWEEN startdt AND enddt) ";
				//AND (".tosql($rs->fields['enddate'])." BETWEEN startdt AND enddt)";
				$rste = $conn->execute($sqlte);
				$cnt = $rste->recordcount();
				if(empty($cnt)){
