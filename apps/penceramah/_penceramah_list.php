...
if(!empty($pro) && $pro=='PILIH'){
	...
	$sqli = "INSERT INTO _tbl_claim (cl_id, cl_ins_id, cl_month, cl_year, 
	create_dt, create_by, is_deleted, is_process)
	VALUES(".tosql($maxno).", ".tosql($ingid).", ".tosql(date("m")).", ".tosql(date("Y")).", 
	".tosql(date("Y-m-d H:i:s")).", ".tosql($ipdaye_by).",0 , 0)";
	$conn->execute($sqli);
	$href_link = "../index.php?data=".base64_encode('user;penceramah/claim_form.php;penceramah;tuntutan;'.$maxno);
	?>
...
<? include_once 'include/page_list.php'; ?>
    ...
    <tr>
        <td colspan="4" align="center">
            <table width="100%" border="1" cellpadding="3" cellspacing="0" bgcolor="#000000">
                <tr bgcolor="#CCCCCC">
                    ...
                    while(!$rs->EOF  && $cnt <= $pg) {
                        ...
                        <tr bgcolor="<? if ($cnt%2 == 1) echo $bg1; else echo $bg2 ?>">
                            <td valign="top" align="right"><?=$bil;?>.</td>
			             	<td valign="top" align="left"><a href="penceramah/_penceramah_list.php?pro=PILIH&ingenid=<?=$rs->fields['ingenid'];?>">
								<? echo stripslashes($rs->fields['insname']);?></a>&nbsp;</td>
