...
$rs = $conn->query($sql);
if($rs->EOF){
	$fset_id = uniqid(date("Ymd-His"));
	$sql_ins = "INSERT INTO _tbl_set_penilaian (fset_id, fset_event_id, fset_create_dt, fset_create_by)
	VALUES(".tosql($fset_id).", ".tosql($kursus_id).", ".tosql(date("Y-m-d H:i:s")).", ".tosql($_SESSION["s_userid"]).")";
	//print "<br>".$sql_ins;
...
$sql1 = "SELECT * FROM _tbl_set_penilaian_peserta WHERE event_id=".tosql($kursus_id)." AND fset_id=".tosql($fset_id)." 
...
if($rs_sql1->EOF){
	...
	/*$sql_ins1 = "INSERT INTO _tbl_set_penilaian_peserta (fset_id, event_id, id_peserta".$umurp.")
		SELECT fset_id, fset_event_id, ".tosql($id)." FROM _tbl_set_penilaian
 		WHERE fset_event_id=".tosql($kursus_id)." AND fset_id=".tosql($fset_id);*/
	$sql_ins1 = "INSERT INTO _tbl_set_penilaian_peserta (fset_id, event_id, id_peserta, fsetp_jawatan, fsetp_negeri, 
		fsetp_tempat_tugas".$umurp.$jantina.$jawatan.$kursus_hadir.")
	VALUES(".tosql($fset_id).", ".tosql($fset_event_id).", ".tosql($id).", ".tosql($positions).", ".tosql($negeri).", 
		".tosql($ttugas).", 1, 1, 1, 1)";
...
$sql_det = "SELECT A.* FROM _tbl_set_penilaian_peserta_bhg A 
WHERE A.event_id=".tosql($kursus_id)." AND A.id_peserta=".tosql($id)." AND A.fset_id=".tosql($fset_id);
$sql_det .= " ORDER BY fsetbp_jadmasaid";
//$conn->debug=true;
