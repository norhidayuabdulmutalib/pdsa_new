...
if($sub_tab=='peserta'){
	...
	$sSQL .= " ORDER BY D.no_bilik, A.f_peserta_nama";
	...
	$sSQL .= $sql_search . " ORDER BY D.no_bilik, A.insname";
	$rs = &$conn->Execute($sSQL);
	$cnt = $rs->recordcount();
}
$conn->debug=false;
      ...
      <td width="75%" colspan="5"> <table border="1" width=100% cellspacing="0" cellpadding="5" bordercolorlight="#000000" bordercolordark="#FFFFFF">
        ...
        if(!$rs->EOF) {
            ...
            while(!$rs->EOF  && $cnt <= $pg) {
				...
				if($sub_tab=='peserta'){
					$kursus = dlookup("_tbl_kursus A, _tbl_kursus_jadual B, _tbl_kursus_jadual_peserta C","A.coursename",
					"A.id=B.courseid AND B.id=C.EventId AND C.StaffNoIC=".tosql($rs->fields['NOKP'],"Text")); 
				} else {
				}
            ?>
          <tr bgcolor="<? if ($cnt%2 == 1) echo $bg1; else echo $bg2 ?>">
            <td align="right" valign="top"><? echo $bil;?>.&nbsp;</td>
            <td valign="top"><a href='<?=$href_link;?>'><? echo stripslashes($rs->fields['daftar_nama']);?></a>
            	<br /><i>[<? echo stripslashes($rs->fields['NOKP']);?>]</i>&nbsp;</td>
            <td align="center"><? echo stripslashes($rs->fields['agensi']);?>&nbsp;</td>
            <td align="center"><? echo stripslashes($kursus);?>&nbsp;</td>
