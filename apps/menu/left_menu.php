...
<? if($_SESSION['menu']=='default'){ ?>
    ...
    <? if(!$rs_menu->EOF){
			while(!$rs_menu->EOF){ ?>
			<li <? if($_SESSION['menu']=='mohon'){ print 'class="current"'; }?>>
				<a href="index.php?data=<? print base64_encode($rs_menu->fields['menu_link']);?>"><b><? print $rs_menu->fields['grp_name'];?></b></a></li>
		<? 		$rs_menu->movenext();
			}
	   } else {
    ...
    ?>
    	<li <? if($_SESSION['menu']=='mohon'){ print 'class="current"'; }?>>
            <a href="index.php?data=<? print base64_encode('user;../include/user_pass.php');?>"><b>Tukar Katalaluan</b></a></li>
        </ul>	
    </div>
<? } else { ?>
    <?
    ...
    <div id="menu12">
        <ul>
		<? while(!$rs_menu->EOF){ ?>
                <li <? if($_SESSION['sub_menu']==$rs_menu->fields['sub_menu']){ print 'class="current"'; }?>><?//=$_SESSION['sub_menu'];?>
                    <a href="index.php?data=<? print base64_encode($rs_menu->fields['menu_link']);?>&dq_cari=M"><b><? print $rs_menu->fields['menu_name'];?></b></a></li>
        <? 		$rs_menu->movenext();
            }
        ?>
        </ul>
