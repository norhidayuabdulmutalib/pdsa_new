...
?>
    ...
    <table width="100%" cellpadding="5" cellspacing="0">
		<tr bgcolor="#009999">
        	<td colspan="5"><b>Nama Kakitangan : <? print stripslashes($rs->fields['fld_staff']);?></b></td>
        </tr>
    	<?  //while($rowm = mysql_fetch_assoc($rsm)){
			while(!$rs_m->EOF){ 
				...
				?>
				<tr bgcolor="#CCCCCC">
					<td colspan="5"><b>Kumpulan Menu : <? print $rs_m->fields['grp_name'];?></b></td>
				</tr>
				<tr>
				...
				?>
				<tr>
					<td align="right">&raquo;&nbsp;<input type="checkbox" name="is_menu[<?=$bil;?>]" <? if($is_menu==1){ print 'checked';}?>>&nbsp;
					<input type="hidden" name="menu_uid[<?=$bil;?>]" value="<?=$menu_uid;?>">
					<input type="hidden" name="menu_id[<?=$bil;?>]" value="<?=$menu_id;?>">
					</td>
					<td align="left"><? print $rsmd->fields['menu_name'];?></td>
					<td align="center"><input type="checkbox" name="is_add[<?=$bil;?>]" value="1" <? if($is_add==1){ print 'checked';}?>></td>
					<td align="center"><input type="checkbox" name="is_upd[<?=$bil;?>]" value="1" <? if($is_upd==1){ print 'checked';}?>></td>
					<td align="center"><input type="checkbox" name="is_del[<?=$bil;?>]" value="1" <? if($is_del==1){ print 'checked';}?>></td>
				</tr>
				<? $bil++; $rsmd->movenext();
				} 
			}?>
