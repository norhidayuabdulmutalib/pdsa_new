...
if(!empty($proses) && $proses=='PRO'){
	...
	$conn->execute("UPDATE _tbl_kursus_jadual SET ref_surat_peserta=".tosql($_POST['surat'])." WHERE id = ".tosql($id,"Text"));
	...
	$rss = $conn->execute("SELECT ref_surat_peserta FROM _tbl_kursus_jadual WHERE id = ".tosql($id,"Text"));
	if(!$rss->EOF){ 
	...
	$surat = str_replace("!tajuk kursus!",$namakursus,$surat);
...
if(!$rss->EOF){
	...
	if(empty($surat)){
		...
		if(!$rs->EOF){ $bil++; 
			$surat = stripslashes($rs->fields['surat_1']);
			//$rs->movenext();
			$sql = "UPDATE _tbl_kursus_jadual SET ref_surat_peserta=".tosql($rs->fields['surat_1'])." WHERE id = ".tosql($id,"Text");
			$conn->execute($sql);
		} 
	}
}
